...
from rlkit.torch.torch_17_distributions.multivariate_normal import (
    _batch_mahalanobis,
    _batch_mv,
)
from rlkit.torch.torch_17_distributions.utils import _standard_normal, lazy_property
...
class LowRankMultivariateNormal(Distribution):
    ...
    def precision_matrix(self):
        ...
        Wt_Dinv = self._unbroadcasted_cov_factor.transpose(
            -1, -2
        ) / self._unbroadcasted_cov_diag.unsqueeze(-2)
        A = torch.triangular_solve(Wt_Dinv, self._capacitance_tril, upper=False)[0]
        precision_matrix = torch.diag_embed(
            self._unbroadcasted_cov_diag.reciprocal()
